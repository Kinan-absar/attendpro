...
const History: React.FC<Props> = ({ history, user, onRefresh }) => {
  ...
  const totalHours = useMemo(() => {
    return filteredHistory.reduce((acc, record) => {
      ...
      const calcDuration = record.checkOut ? (record.checkOut.getTime() - record.checkIn.getTime()) / 60000 : 0;
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12">
        ...
        <div className="flex flex-wrap items-end gap-3 bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-100 flex items-center space-x-2 active:scale-95 transition-all"
            >
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden card">
        {filteredHistory.length === 0 ? (
          <div className="p-20 text-center">
            ...
            <p className="text-slate-400 font-medium tracking-tight">No records found for the selected period.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredHistory.map((record) => {
                  ...
                  return (
                    <tr key={record.id} className={`hover:bg-slate-50/30 transition-colors ${record.needsReview ? 'bg-rose-50/20' : ''}`}>
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex flex-wrap justify-center gap-1">
                          {record.autoClosed && (
                            <span className="px-2 py-0.5 bg-rose-100 text-rose-700 text-[8px] font-black uppercase rounded border border-rose-200">Auto-Closed</span>
                          )}
                          {record.needsReview && !record.autoClosed && (
                            <span className="px-2 py-0.5 bg-orange-100 text-orange-700 text-[8px] font-black uppercase rounded border border-orange-200">Flagged</span>
                          )}
                          {!record.needsReview && record.checkOut && (
                            <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[8px] font-black uppercase rounded border border-emerald-200">Verified</span>
                          )}
                          {!record.checkOut && <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-[8px] font-black uppercase rounded border border-indigo-200">Active</span>}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        {dataService.getCurrentUser()?.role === 'admin' && (
                          <button onClick={() => setEditingRecord(record)} className="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:text-indigo-600 transition-all shadow-sm">
      ...
      {/* Signature Section for Printed Sheet */}
      <div className="hidden print:flex mt-16 justify-between items-end gap-12">
        <div className="flex-1 border-t border-slate-900 pt-3">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-8">Employee Signature</p>
        </div>
        <div className="flex-1 border-t border-slate-900 pt-3">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-8">Site Manager Approval</p>
        </div>
        <div className="flex-1 border-t border-slate-900 pt-3">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-8">Human Resources Seal</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-fadeIn">
            <div className="bg-indigo-600 p-8 text-white flex justify-between items-center">
              <div>
                <h2 className="text-xl font-black tracking-tight">Correct Shift Data</h2>
                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-100 opacity-70 mt-1">Admin Audit Override</p>
              </div>
            ...
            <form onSubmit={handleUpdate} className="p-8 space-y-6">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Clock In Timestamp</label>
                <input name="checkIn" type="datetime-local" defaultValue={formatDateForInput(editingRecord.checkIn)} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Clock Out Timestamp</label>
                <input name="checkOut" type="datetime-local" defaultValue={editingRecord.checkOut ? formatDateForInput(editingRecord.checkOut) : ''} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
              </div>
              <div className="pt-4 flex gap-4">
                <button type="button" onClick={() => setEditingRecord(null)} className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all">Cancel</button>
                <button disabled={saving} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all disabled:opacity-50">
                  {saving ? <i className="fa-solid fa-circle-notch fa-spin"></i> : 'Publish Correction'}
                </button>
              </div>
            </form>
          </div>
